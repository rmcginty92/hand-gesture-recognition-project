...
int filter(unsigned char *input, unsigned char *output) {
   int ii, jj, kk, ll,h_len; 
	//h_len = 7;
   // 7x7 Guassian lpf generated with sigma = 3
	//float h[49] = { 	0.011362, 0.014962, 0.017649, 0.018648, 0.017649, 0.014962, 0.011362, \
   ...
   h_len = 3;
   //float h[9] = {0, -1, 0, -1, 5, -1, 0, -1, 0};
   float h[9] = {-1, -1, -1, -1, 15, -1, -1, -1, -1};

   int rbuffer[WIDTH+h_len-1][HEIGHT+h_len-1],gbuffer[WIDTH+h_len-1][HEIGHT+h_len-1],bbuffer[WIDTH+h_len-1][HEIGHT+h_len-1];
   int tempr, tempg, tempb;


   
   float h_norm[h_len*h_len];
   norm(h, h_norm,h_len);


   for(ii=0;ii<WIDTH+h_len-1;ii++){
      for(jj=0;jj<HEIGHT+h_len-1;jj++){
         if(ii==0 || jj==0 || ii==WIDTH+1 || jj==HEIGHT+1){
            ...
            rbuffer[ii][jj] = (int)input[3*((ii-1)+(jj-1)*WIDTH)+0];
	    		gbuffer[ii][jj] = (int)input[3*((ii-1)+(jj-1)*WIDTH)+1];
            bbuffer[ii][jj] = (int)input[3*((ii-1)+(jj-1)*WIDTH)+2];
	    		//printf("uchar%d int%d\n",input[3*((ii-1)*WIDTH+(jj-1))+0],rbuffer[jj][ii]);
         }
      }
   }
	...
	for(ii=0;ii<WIDTH;ii++){
		for(jj=0;jj<HEIGHT;jj++){
			...
			tempg = 0;
			tempb = 0;
			for(kk=0;kk<h_len;kk++){
				for(ll=0;ll<h_len;ll++){
					tempr += rbuffer[ii+kk][jj+ll]*h_norm[kk*3+ll];
					tempg += gbuffer[ii+kk][jj+ll]*h_norm[kk*3+ll];
					tempb += bbuffer[ii+kk][jj+ll]*h_norm[kk*3+ll];
				} 
			}
			output[3*(ii+jj*WIDTH)+0] = (unsigned char)tempr;
			output[3*(ii+jj*WIDTH)+1] = (unsigned char)tempg;
			output[3*(ii+jj*WIDTH)+2] = (unsigned char)tempb;
		}	
   }
...
int norm(float* input, float* output,int h_len){
	float power=0;
	int ii;
	for(ii=0;ii<h_len*h_len;ii++){
		power += input[ii];
	}
	for(ii=0;ii<h_len*h_len;ii++){
...
int main(){
   ...
   filter(rgb, filtered_image);
	FILE* out1 = fopen("filtered_image.jpg", "w");
	jpeg(out1, filtered_image, camera->width, camera->height, 100);
	free(filtered_image);
	fclose(out1);
	FILE* out2 = fopen("original.jpg", "w");
